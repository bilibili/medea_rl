...
async def single_compute_score(evaluation_func, completion, reference, task, task_extra_info, executor, timeout=600, log_print=False):
...
async def parallel_compute_score_async(evaluation_func,
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        # Create tasks for all rows
        tasks_async = [
            single_compute_score(evaluation_func, completion, reference, task, task_extra_info, executor, timeout=600)
            for completion, reference, task, task_extra_info in zip(completions, references, tasks, extra_info)
        ]
        # to prevent very occasional starvation caused by some anomalous programs ( like infinite loop ), the exceptions in async programs will instantly halt the evaluation, and all summoned processes will be killed.
        try:
            ...
            # 线程池的进程 kill 不需要像ProcessPool那样处理，但仍保留捕获逻辑
            raise
...
class IndexRewardManager:
    ...
    def __init__(self, tokenizer, num_examine, compute_score=None, reward_fn_key: str="data_source", \
        ...
        self.reward_num_processes = reward_num_processes
        self.max_resp_len = max_resp_len
        # self.grm_cfg = NestedNamespace(grm)
        # self.overlong_buffer_cfg = NestedNamespace(overlong_buffer)

        # self.use_llm_verify = self.grm_cfg.use_llm_verify
        # self.llm_server_ip = self.grm_cfg.llm_server_ip
        # self.llm_server_port = self.grm_cfg.llm_server_port
        # self.genRM_app_address = self.grm_cfg.genRM_app_address
        # self.instruct_follow_app_address = self.grm_cfg.instruct_follow_app_address
        # self.reward_model_app_address = self.grm_cfg.reward_model_app_address
        ...
        # print("[PrimeRewardManager] Embedding model loaded on", self.embedding_device)
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        ground_truth = [
            data_item.non_tensor_batch.get('reward_model', {}).get('ground_truth', '')
        ...
        try:
            scores = asyncio.run(
                parallel_compute_score_async(self.compute_score,
                                             ...
                                             num_workers=self.reward_num_processes))
        ...
        for i in range(len(scores)):
            ...
            if 'extra_info' in scores[i]:
                ...
                r_collection = scores[i]['extra_info']['r_collection']
                r_rank = scores[i]['extra_info']['r_rank']
            ...
            reward_extra_info["r_collection"].append(r_collection)
            reward_extra_info["r_rank"].append(r_rank)
