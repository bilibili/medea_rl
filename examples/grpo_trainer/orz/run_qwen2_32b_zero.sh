...
RESUME_MODEL_DOWNLOAD_PATH="${RESUME_MODEL_DOWNLOAD_PATH:-}"

DATA_DOWNLOAD_PATH="${DATA_DOWNLOAD_PATH:-s3://llm-data/benjamin/datasets/Open-Reasoner}"
DATA_PATH="${DATA_PATH:-/workspace/data/Open-Reasoner}"
TRAIN_FILE="${TRAIN_FILE:-train.parquet}"
TEST_FILE="${TEST_FILE:-test.parquet}"
...
# pip install vllm==0.7.3 -i https://pypi.bilibili.co/repository/pypi-public/simple
...
# 下载checkpoint for resume
if [ -n "$RESUME_MODEL_DOWNLOAD_PATH" ]; then
    python3 verl/bl_utils/boss_transfer.py \
        --config=/workspace/verl/boss_credentials.toml \
        --source="${RESUME_MODEL_DOWNLOAD_PATH}" \
        --dest="${RESUME_MODEL_PATH}"

    _EXTRA_ARGS+=( "++trainer.resume_from_path=True" )
    _EXTRA_ARGS+=( "++trainer.resume_mode=${RESUME_MODEL_PATH}" )
else
    echo "环境变量 RESUME_MODEL_DOWNLOAD_PATH 不存在，跳过checkpoint下载程序。"
fi
...
if [ "${TASK_TYPE}" == "master" ]; then
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        data.train_files="${DATA_PATH}/${TRAIN_FILE}" \
        data.val_files="${DATA_PATH}/${TEST_FILE}" \
