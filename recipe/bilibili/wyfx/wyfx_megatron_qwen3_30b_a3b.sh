...
_MAX_PROMPT_L="${MAX_PROMPT_L:-4096}"
_MAX_RES_L="${MAX_RES_L:-2048}"
...
MODEL_ARGS=()
MODEL_ARGS+=(
    actor_rollout_ref.model.path=/mnt/group/opensource_models/Qwen3-30B-A3B
    ...
    reward_model.reward_kwargs.reward_num_processes=8
...
if [ "${TASK_TYPE}" == "master" ]; then
    python3 -m recipe.bilibili.wyfx.main_ijp \
            ...
            ${PLACEHOLD_ARGS[@]} \
            ${MEGATRON_OVERRIDE_TRANSFORMER_CONFIG[@]} \
            $@ 2>&1 | tee verl_wyfx_qwen3_a3b.log
fi
...
# Usage Example:
# Start up container:
# hub.bilibili.co/nyx-base/javis-megatron:qwen3_next_2
#
# ```
# cd /workspace/verl
# pip install -r /mnt/group/benjamin/data/requirements_for_verl_1020.txt
# bash recipe/bilibili/wyfx/wyfx_megatron_qwen3_30b_a3b.sh \
